...
# Comment type
include_types = ['enhancer',                      #1
                 # 'promoter',                      #2
                 'pseudogene',                    #3
                 'insulator',                     #4
                 'conserved_region',              #5
                 'protein_binding_site',          #6
                 'DNAseI_hypersensitive_site',    #7
                 'nucleotide_cleavage_site',      #8
                 'silencer',                      #9
                 'gene',                          #10
                 'exon',                          #11
                 'CDS',                           #12
...
def load_npz_data_for_classification(file_name, ngram=3, only_one_slice=True, ngram_index=None, masked=True):
    ...
    print("Annotation: ", anno_data.shape)
    ...
    if only_one_slice is True:
        for ii in range(ngram):
            ...
            anno_data_slice = anno_data[:, :, slice_indexes]
            x_data_all.append(x_data_slice)
            anno_data_all.append(anno_data_slice)
            y_data_all.append(y_data)
    else:
        x_data_all.append(x_data)
        anno_data_all.append(anno_data)
        y_data_all.append(y_data)
...
def load_all_data(record_names: list, ngram=3, only_one_slice=True, ngram_index=None, masked=False):
    ...
    for file_name in record_names:
        ...
        anno_data_all.extend(anno_data)
        y_data_all.extend(y_data)

    x_data_all = np.concatenate(x_data_all)
    anno_data_all = np.concatenate(anno_data_all)
...
def load_npz_dataset_for_classification(x_promoter_data_all,
    ...
    def data_generator():
        ...
        while True:
            ...
            annotation = annotation_data_all[index]
            segment_promoter = np.zeros_like(x_promoter)

            y = y_data_all[index]
            ii += 1
            yield x_promoter, segment_promoter, annotation, y
    ...
    dataset = tf.data.Dataset.from_generator(data_generator,
                                             output_types=(tf.int16, tf.int16, tf.int16, tf.int16),
                                             output_shapes=(
...
def parse_function(x_promoter, segment_x, annotations, y):
    x = {
    ...
    for ii in range(annotations.shape[1]):
        ...
        if ii < annotations.shape[1] and ii < annotation_size:
            if ii in exclude_annotation:
                gene_type = K.zeros_like(annotations[:, ii, :], dtype='int64')
            else:
                gene_type = annotations[:, ii, :]

            x['Input-Token-Type_{}'.format(index)] = gene_type
            index += 1
...
def train_kfold(train_data_file,
    ...
    only_one_slice = True
    region1_seq, annotation, label = load_all_data(train_promoter_files, ngram=ngram, only_one_slice=only_one_slice,
                                       ngram_index=None)
    ...
    for train, test in kfold.split(X, Y):
        ...
        anno_train_data = Annotation[train[train_slice]]
        y_train_data = Y[train[train_slice]]

        x_valid_data = X[train[valid_slice]]
        anno_valid_data = Annotation[train[valid_slice]]
        y_valid_data = Y[train[valid_slice]]

        x_test_data = X[test]
        anno_test_data = Annotation[test]
        y_test_data = Y[test]

        np.savez('./data/kfold_{}_train_and_valid_index_{}_gram_{}_11_Knowledge.npz'.format(str(k_fold), str(ngram), task_name), train=train,
                 test=test)
        ...
        filename = './data/promoter_best_model_gene_bert_{}_fold_{}_gram_{}_11_Knowledge.h5'.format(str(k_fold), str(ngram), task_name)

        modelCheckpoint = ModelCheckpoint(filename, monitor='val_acc', save_best_only=True, verbose=1)

        print('fitting...')
        region1_seq = x_train_data
        region1_seq_valid = x_valid_data
        ...
        # EVAL_ONLY: model_train_history = model.fit(train_dataset,
        # EVAL_ONLY: steps_per_epoch=train_steps_per_epoch,
        # EVAL_ONLY: epochs=epochs,
        # EVAL_ONLY: validation_data=valid_dataset,
        # EVAL_ONLY: validation_steps=valid_steps_per_epoch,
        # EVAL_ONLY: callbacks=[modelCheckpoint, early_stopping],
        # EVAL_ONLY: verbose=2)
        # EVAL_ONLY:         # EVAL_ONLY: print(model_train_history)
...
if __name__ == '__main__':
    ...
    # task_name=task_name)
    # TATA BOX
    # train_data_file = 'epdnew_TATA_BOX_Knowledge_5_gram.npz'
    # data_path = './data/' + '{}_gram_11_knowledge'.format(ngram)
